...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function mostrarApp(username, empresaGuardada) {
    ...
    .then(res => res.json())
    .then(empresas => {
      empresaSelect.innerHTML = `<option value="">-- Seleciona Empresa --</option>`;
      empresas.forEach(e => {
      ...
      if (empresaAtual) empresaSelect.value = empresaAtual;
    });
    ...
    function guardarDado(valor) {
      ...
      fetch(`${SUPABASE_URL}/rest/v1/dados?numero=eq.${encodeURIComponent(dado.numero)}&utilizador=eq.${encodeURIComponent(username)}`, {
        headers: {
          apikey: SUPABASE_KEY,
          Authorization: SUPABASE_AUT
        }
    ...
    document.addEventListener("click", (e) => {
      if (e.target.classList.contains("btn-eliminar")) {
        ...
        if (confirm("Eliminar este registo?")) {
          fetch(`${SUPABASE_URL}/rest/v1/dados?id=eq.${id}`, {
            method: "DELETE",
            headers: {
              apikey: SUPABASE_KEY,
              Authorization: SUPABASE_AUT
